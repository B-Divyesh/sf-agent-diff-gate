...
test('@claim:stateful-worker-deploy factory post-turn deployment delegates to the stateful release', async () => {
...
test('factory deployment hook rejects a generic or misrouted release', () => {
  const wrongProduct = spawnSync(hook, ['other-product', '/work/repo', 'Dockerfile', '8080'], {
    encoding: 'utf8',
  });
  assert.equal(wrongProduct.status, 2);
  assert.match(wrongProduct.stderr, /unexpected product/);

  const wrongPort = spawnSync(hook, ['agent-diff-gate', '/work/repo', 'Dockerfile', '3000'], {
    encoding: 'utf8',
  });
