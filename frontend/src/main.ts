...
type Draft = {
  ...
  approved_by?: string;
  approved_at?: string;
  test_evidence?: { command: string; result: string; recorded_by: string; recorded_at: string };
  github_revision?: { head_sha: string; previous_head_sha?: string };
};
type PolicyRule = { path: string; required_owner: string };
type RepositoryPolicy = { repository: string; rules: PolicyRule[] };
type Auth = {
...
function shell(content: string, page: string, title: string, description: string) {
  ...
  const image = `${location.origin}/social.webp`;
  document.querySelector<HTMLMetaElement>('meta[property="og:image"]')!.content = image;
  ...
  document.querySelector<HTMLMetaElement>('meta[name="twitter:image"]')!.content = image;
...
function packetUI() {
  ...
  const hasRecordedTestEvidence = Boolean(draft.test_evidence);
...
function notFound() {
  shell(
    `<section class="not-found"><p class="eyebrow">404</p><h1 tabindex="-1">Page not found</h1><p>That page does not exist. Return to the packet list to continue reviewing.</p><button class="primary" id="go-home">Go to Diff Gate</button></section>`,
    "Not found — Diff Gate",
    "Not found — Diff Gate",
  ...
  try {
    const saved = await api<any>("/api/packets", {
      ...
      body: JSON.stringify({
        ...
        test_evidence: evidence ? { command: "Manual test evidence", result: evidence } : undefined,
      }),
    });
    draft = {
...
async function saveEvidence(testEvidence?: { command: string; result: string }) {
...
function bind() {
  ...
  document
    ...
    ?.addEventListener("submit", async (event) => {
      event.preventDefault();
      const note = document.querySelector("#real-note")!;
      const repository = document.querySelector<HTMLInputElement>("#policy-repository")!.value;
      const rules = document.querySelector<HTMLTextAreaElement>("#policy-rules")!.value
        .split("\n")
        .map((line) => line.split("|").map((value) => value.trim()))
        .filter(([path, owner]) => path && owner)
        .map(([path, required_owner]) => ({ path, required_owner }));
      try {
        const saved = await api<RepositoryPolicy>("/api/repository-policies", {
          method: "PUT",
          body: JSON.stringify({ repository, rules }),
        });
        repositoryPolicies = [...repositoryPolicies.filter((policy) => policy.repository !== saved.repository), saved]
          .sort((a, b) => a.repository.localeCompare(b.repository));
        render();
      } catch (error) {
        note.textContent = error instanceof Error ? error.message : "Could not save the repository policy.";
      }
    });
  document
  ...
  document
    .querySelector<HTMLFormElement>("#test-evidence-form")
    ?.addEventListener("submit", async (event) => {
      event.preventDefault();
      const command = document.querySelector<HTMLInputElement>("#evidence-command")!.value;
      const result = document.querySelector<HTMLTextAreaElement>("#evidence-result")!.value;
      try {
        await saveEvidence({ command, result });
        render();
      } catch (error) {
        const note = document.querySelector("#approval-note")!;
        note.textContent = error instanceof Error ? error.message : "Could not save test evidence.";
      }
    });
  document
  ...
  document.querySelector("#refresh-github")?.addEventListener("click", async () => {
    if (!draft?.id || demo) return;
    try {
      const saved = await api<any>(`/api/packets/${draft.id}/refresh`, { method: "POST" });
      draft = {
        ...JSON.parse(saved.data), id: saved.id, title: saved.title, owner: saved.owner,
        status: saved.status, source_url: saved.source_url, approved_by: saved.approved_by,
        approved_at: saved.approved_at,
      };
      packetNotice = "GitHub revision checked. Review the current evidence before approval.";
      await loadAudit(saved.id);
      render();
    } catch (error) {
      const note = document.querySelector("#approval-note")!;
      note.textContent = error instanceof Error ? error.message : "Could not refresh this pull request.";
    }
  });
