...
for (const profile of [
  ...
  if (missingResponse?.status() !== 200 || missingResponse.headers()['x-diff-gate-route'] !== 'not-found' || missingResponse.headers()['x-robots-tag'] !== 'noindex') {
    throw new Error(`${profile.name}: unknown route did not return the noindex recovery contract`);
  }
